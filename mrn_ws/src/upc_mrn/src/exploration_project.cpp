...
//         define them HERE
//////////////////////////////////////////////////////////////////////
int max_size=9999999;
int id_frontier;
float dist_i;
int count_err=0;
int size_threshold=9;
int cuenta=0;
//////////////////////////////////////////////////////////////////////
// TODO 1 END
//////////////////////////////////////////////////////////////////////
...
geometry_msgs::Pose decideGoal()
{
  ...
  for (int i=0;i<frontiers_.size();i++){
    if(frontiers_[i].size > size_threshold){
    dist_i = ((frontiers_[i].free_center_point.x - robot_pose_.position.x)*(frontiers_[i].free_center_point.x - robot_pose_.position.x))+((frontiers_[i].free_center_point.y - robot_pose_.position.y)*(frontiers_[i].free_center_point.y - robot_pose_.position.y))*0.75-frontiers_[i].size*0.25;
    if (dist_i < max_size && frontiers_[i].size >5){
      max_size=dist_i;
      id_frontier=i;
    }
  }
  ...
  //EXAMPLE: Choose valid random goal
      ...
      g.orientation = tf::createQuaternionMsgFromYaw(rand_yaw);
      g.position.x = frontiers_[id_frontier].free_center_point.x;
      g.position.y = frontiers_[id_frontier].free_center_point.y;
      if(robot_pose_.position.x==prev_robot_pose_.position.x && robot_pose_.position.y==prev_robot_pose_.position.y)
      {
        cuenta+=1;
        if (cuenta>=3){
          geometry_msgs::Pose g_stuck;
          g_stuck = getRandomPose(0.5);
          g.position.x = robot_pose_.position.x+g_stuck.position.x;
          g.position.y = robot_pose_.position.y+g_stuck.position.y;
          cuenta=0;
        }
      }
